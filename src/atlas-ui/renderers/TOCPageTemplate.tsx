...
export function TOCPageTemplate({ page, locale, readingOrder, getPage }: TOCPageTemplateProps) {
        ...
        <ol className="space-y-3">
          {items.map((p, i) => (
            <li key={p.pageId} className="flex items-baseline gap-4">
              <span className="text-stone-500 text-sm w-8 text-right shrink-0">
                {p.pageNumber ?? i + 1}
              </span>
              <Link
                to={`/page/${p.pageId}`}
                className="text-stone-300 hover:text-stone-100 hover:underline text-lg"
              >
